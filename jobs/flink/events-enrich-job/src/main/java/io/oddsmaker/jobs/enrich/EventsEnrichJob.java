...
public class EventsEnrichJob {
    static final String JOB_NAME = "oddsmaker-events-enrich";
    ...
    static String[] config() {
        return new String[]{
                ...
                System.getProperty("geoip.mmdb", ""),
                System.getProperty("kafka.dlq", "oddsmaker.deadletter"),
        };
    }
    ...
    /**
     * 搭建清洗/富化管道（惰性：source/sink 均到 execute 才连接，单测可用本地环境直跑）。
     */
    static SingleOutputStreamOperator<EventRow> buildPipeline(StreamExecutionEnvironment env, String[] cfg) {
        ...
        SingleOutputStreamOperator<RawEvent> validated = stream.process(new ValidateFunction(DLQ));
        ...
        // Optional enrichers（真对象在算子端懒初始化）
        final Enrichers enrichers = Enrichers.create(cfg[6]);

        SingleOutputStreamOperator<EventRow> mapped = deduped
                .map((org.apache.flink.api.common.functions.MapFunction<RawEvent, EventRow>) record -> toRow(record, enrichers))
    ...
    /** 基础校验：必填字段缺失走 DLQ，ts_server 缺失填 now。 */
    static class ValidateFunction extends ProcessFunction<RawEvent, RawEvent> {
        private final OutputTag<String> dlq;

        ValidateFunction(OutputTag<String> dlq) {
            this.dlq = dlq;
        }
    ...
    /** RawEvent → events 表行：字段归位 + 国家/UA 富化。 */
    static EventRow toRow(RawEvent record, Enrichers enrichers) {
    ...
    static Timestamp toTimestamp(Long micros) {
    ...
    // geoip 用 GeoIp2Provider 接口类型（DatabaseReader 即其实现），便于替身注入
    static class Enrichers implements java.io.Serializable {
        private final String mmdbPath;
        private transient GeoIp2Provider geoip;
        ...
        /** 三族字段解析合一：AgentName / OperatingSystemName / DeviceClass。 */
        private String uaValue(String ua, String fieldName) {
            ensureInit();
            if (uaa == null || ua == null || ua.isEmpty()) return null;
            try {
                UserAgent parsed = uaa.parse(ua);
                return parsed.getValue(fieldName);
            } catch (Exception e) { return null; }
        }

        String uaFamily(String ua) { return uaValue(ua, "AgentName"); }

        String osFamily(String ua) { return uaValue(ua, "OperatingSystemName"); }

        String deviceClass(String ua) { return uaValue(ua, "DeviceClass"); }
    }

    // Merge a single string key/value into existing JSON object string; fall back to concat if invalid
    static String mergeProps(String json, String key, String value) {
        try {
