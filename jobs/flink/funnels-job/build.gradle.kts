...
dependencies {
  ...
  // ConfigurableFunnelsJob 启动时从 control PG 加载漏斗配置
...
// 模块内有两个 main：固定两步漏斗 FunnelsJob（Main-Class）与
// ConfigurableFunnelsJob（从 control PG 加载漏斗配置，用 java -cp <jar> <主类> 方式运行）
val fatJar = tasks.register<Jar>("fatJar") {
  archiveClassifier.set("all")
  duplicatesStrategy = DuplicatesStrategy.EXCLUDE
  manifest { attributes["Main-Class"] = "io.oddsmaker.jobs.funnels.FunnelsJob" }
